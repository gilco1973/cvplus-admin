...
export const getUserPolicyViolations = onCall({
    cors: true
}, async (request) => {
    ...
    try {
        ...
        const uniqueWarnings = warnings.filter((warning, index, arr) => {
            const duplicateIndex = arr.findIndex(w => w.type === warning.type &&
        ...
        throw new HttpsError('internal', 'Failed to get user policy violations', error);
    }
});
//# sourceMappingURL=getUserPolicyViolations.js.map